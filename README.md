...
| --- | --- | --- | --- | --- | --- |
| `boto3` | `list/scan/query operations` | **9.7y** — present at all 11 releases sampled from 1.4.4 (2017-01-16) th... | the caller's result set fits in one page | list_objects_v2 returned 1,000 of 2,500 objects — 40% of the truth, 1,500 silently missing, 0 exceptions and 0 warnings, HTTP 200 and a well-formed... | Very poor, and there is no ceiling to learn... |
| `charset-normalizer` | `detect / from_bytes(...).best()` | **6.9y** — present in all 16 releases measured from 0.3.0 (2019-09-12) t... | a successful decode is evidence the encoding is right — true for UTF-8,... | 20 sentences in 20 legacy encodings: 8 round-trip to a DIFFERENT string (40%) and only 6 of 20 named the right encoding, with 0 exceptions and 0 wa... | Effectively none, and the confidence field ... |
| `cryptography` | `x509.Certificate.not_valid_after` | **2.4y** — silent in every release measured from 3.4.8 (2021-08-24) thro... — **fixed in 42.0.0** | the caller will compare this naive UTC value against another UTC value ... | The error equals the host's UTC offset, up to 14 hours in either direction | None on the affected range, and the naive c... |
| `idna` | `encode vs the stdlib 'idna' codec` | **12.4y** — present, with all 12 per-domain outputs byte-for-byte identic... | encoding a domain name is deterministic — one input, one host | 7 of 12 internationalised domains (58%) encode differently under the two standards, and 5 of those produce two separately registrable names: 'stras... | Very poor, and the usual test data hides it |
| `imbalanced-learn` | `SMOTE` | **6.5y** — present, stable to four significant figures, in all 10 releas... | the caller consumes rankings, not probabilities | PR-AUC down 3-24% (0/20 seeds winning at three of four ratios) and calibration destroyed at every ratio on every seed: Brier +41% to +1232% | Poor |
| `optuna` | `pruners.MedianPruner` | **6.6y** — present in all 5 releases measured from 1.0.0 (2020-01-14) th... | early rank predicts final rank | +6379% quality cost on 12/12 seeds when eventual winners look worst early, while saving MORE compute than in the favourable case (63% vs 51%) | Only by re-running without the pruner and c... |
| `packaging` | `SpecifierSet` | **11.7y** — present from 14.3 (2014-11-20) — the first release that shipp... — **PARTLY fixed 26.0** | the caller cares only about ordering — not whether a candidate is a pre... | 30% of 60 specifier/version pairs disagree with plain ordering, in both directions | Poor in both directions |
| `pandas` | `DataFrame.merge` | **6.8y** — present and silent in 12 releases sampled from 0.25.2 (2019-1... | the join key is unique on at least one side | 2.02x row inflation and +105.4% on the summed column, with 0 exceptions, 0 warnings, no nulls introduced, dtypes preserved and every value in range | Poor, and worse than it looks |
| `python-dateutil` | `parser.parse` | **12.0y** — present in all 14 releases measured from 2.1 (2012-03-28) thr... | month-before-day ordering, decided per string rather than per column | 37.2% of a 2,000-row day-first column silently wrong, median 118 days off, 0 exceptions and 0 warnings; one column returned under two conventions (... | Poor |
| `requests` | `Response.text` | **12.4y** — present, silent, and identical on every defect-specific measu... | a text/* response that omits charset is Latin-1, per RFC 2616 §3.7.1 — ... | Over 12 UTF-8 bodies served as text/plain with no charset, measured on the wire: r.text was correct 0/12, r.apparent_encoding was correct 12/12 | Worst case, and the reason it reaches produ... |
| `scikit-learn` | `model_selection.train_test_split` | **6.5y** — present in all 8 releases measured from 0.22.2.post1 (2020-03... | rows are independent — that no two rows share a subject | accuracy overstated by 14.8% and ROC-AUC by 9.5% on 12 of 12 trials (0.9472 vs 0.8250, 0.9910 vs 0.9047) with 150 subjects at 8 rows each, 0 except... | The failure looks like SUCCESS, which is th... |
| `urllib3` | `util.retry.Retry` | **12.2y** — present at 11 releases measured (plus 5 added by independent ... | the caller wants connection-level retries only, spaced by nothing, on m... | Measured on the wire against a local server returning 503 and counting the requests that arrive: Retry(total=3) sends 1 request | Poor for the first, and inverted for the third |
<!-- END CATALOGUE TABLE -->
